...
Tested on Windows/Linux/MacOS/Android/iOS/HTML with Unity 2019.4.21f1
...
A nano node is required to be running which the websocket & http server (for RPC request) will communicate with. Websocket & RPC should be enabled in the `node-config.toml` nano file. 
...
`NanoDemo.cs` contains a sample scene which illustrates all the functionality available:  

To set up using the Nano plugin copy the plugin dlls. A simple example of setting up the necessary functions is:
...
- There is an index out of bounds error when using the Websocket with HTML (WebAssembly) so that is not currently supported, but other RPC functionality is.
